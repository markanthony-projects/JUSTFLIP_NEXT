...
let CACHED_CITY_LIST: string[] | null = null;

export async function parseSlugToFilters(slugArray: string[]): Promise<Record<string, any> | null> {
  if (!slugArray || slugArray.length === 0) return null;

  let dynamicCityList: string[] = [];
  try {
    if (CACHED_CITY_LIST) {
      dynamicCityList = CACHED_CITY_LIST;
    } else {
      const response = await SiteService.fetchPopularCities({ offset: 0, limit: 100 });
      dynamicCityList = (response?.cities || []).map((c: any) => c?.name?.toLowerCase()).filter(Boolean);
