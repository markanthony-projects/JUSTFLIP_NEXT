...
//   const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>, index: number | null = null) => {
...
//       if (uploadedUrl) {
//         setFormData((prevData: any) => ({ ...prevData, [name]: uploadedUrl }));
...
//   const handleRERARadioChange = (e: React.ChangeEvent<HTMLInputElement>) => {
//     setReraSelected(true);
//     if (e.target.value === "yes") {
//       setShowRERAInput(true);
//     } else {
//       setShowRERAInput(false);
//       setFormData((prev: any) => ({ ...prev, rera: "" }));
//     }
//   };


//   const handleExpertiseCheckboxChange = (e: React.ChangeEvent<HTMLInputElement>) => {
//     const { value, checked } = e.target;
//     setFormData((prevData: any) => {
//       const prevArray = prevData.expertiesIn || [];
//       let updatedArray;
//       if (checked) updatedArray = [...prevArray, value];
//       else updatedArray = prevArray.filter((item: any) => item !== value);
...
//   const handleOperationAreaInput = (e: React.ChangeEvent<HTMLInputElement>, index: number) => {
...
//     });
//     const filtered = locations.filter((loc: any) => loc.name.toLowerCase().includes(inputValue.toLowerCase()));
//     setFilteredSuggestions(filtered);
//   };


//   const handleSuggestionSelect = (index: number, selectedLoc: any) => {
//     setFormData((prev: any) => {
//       const updatedCities = [...prev.brokerOperatedCities];
//       const updatedAreas = [...updatedCities[0].operatedAreas];
...
//       setFormData((prev: any) => ({ ...prev, brokerOperatedCities: updatedCities }));
//     }
//   };
...
export default function RegistrationForm() {
  ...
  return (
    <div className="py-8 px-4 md:px-10 bg-white flex flex-col items-center">
      <div className="w-full max-w-4xl">
        <div className="md:hidden mb-4 text-center animate-in fade-in slide-in-from-bottom-2 duration-500">
          <h1 className="text-2xl font-bold text-primary mb-1">Sign Up as Broker</h1>
          <p className="text-xs font-medium text-slate-500">
            Already registered? Please{" "}
            <Link href="/login" className="font-semibold underline text-primary hover:opacity-80" style={{ textDecorationColor: 'var(--secondary, #FFD600)' }}>
              login
            </Link>{" "}
            here.
          </p>
          ...
          {steps.map((label, index) => {
            ...
            return (
              <div key={index} className="flex flex-col items-center text-center bg-white z-10 relative">
                ...
                <span className={`text-xs lg:text-sm mt-2 font-semibold transition-colors duration-300 ${isActive ? 'text-primary' : 'text-slate-400'}`}>
                  {label}
                </span>
              </div>
            )
        ...
        <div className="mt-10 w-full">
          ...
          {/* Navigation Buttons */}
          <div className="flex justify-end mt-3 pt-4 gap-4">
            {currentStep > 0 && (
              <button
                type="button"
                onClick={handlePrevious}
                className="py-2.5 px-6 text-sm font-bold rounded-lg text-primary border border-slate-200 hover:bg-slate-50 transition active:scale-95 outline-none cursor-pointer"
              >
                Previous
              </button>
            )}
            ...
            {currentStep < steps.length - 1 && (
              <button
                type="button"
                onClick={handleNext}
                className="py-2.5 px-6 text-sm font-bold rounded-lg text-white bg-primary transition active:scale-95 outline-none shadow-sm hover:opacity-90 cursor-pointer"
              >
                Next
              </button>
            )}
            ...
            {currentStep === 2 && (
              <button
                type="submit"
                onClick={handleSubmit}
                className="py-2.5 px-6 text-sm flex justify-center items-center font-bold rounded-lg text-white bg-primary transition active:scale-95 outline-none shadow-md hover:opacity-90 cursor-pointer"
              >
                Review Data
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-4 h-4 ml-2">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
