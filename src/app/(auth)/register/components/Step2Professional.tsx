...
export interface Step2ProfessionalProps {
  formData: RegistrationFormData;
  errors: Record<string, string>;
  handleChange: (e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => void;
  handleRERARadioChange: (e: ChangeEvent<HTMLInputElement>) => void;
  showRERAInput: boolean;
  handleCitySelect: (e: ChangeEvent<HTMLSelectElement>) => void;
  cities: any[];
  handleAddField: () => void;
  handleOperationAreaInput: (e: ChangeEvent<HTMLInputElement>, index: number) => void;
  showSuggestionsIndex: number | null;
  setShowSuggestionsIndex: (index: number | null) => void;
  filteredSuggestions: any[];
  handleSuggestionSelect: (index: number, loc: any) => void;
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 md:grid-cols-2 gap-x-6 gap-y-5 animate-in fade-in slide-in-from-right-4 duration-300">
      ...
      <div className="col-span-1 lg:col-span-2 flex flex-col gap-3 mt-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {formData?.brokerOperatedCities?.[0]?.operatedAreas?.map((area: any, index: number) => {
              ...
              <div key={index} className="relative">
                ...
                {citySelected && showSuggestionsIndex === index && filteredSuggestions.length > 0 && (
                  <ul className="absolute z-20 bg-white border border-gray-200 rounded-md w-full mt-1 max-h-40 overflow-y-auto text-xs shadow-lg">
                    {filteredSuggestions.map((loc: any) => (
                      <li
                        key={loc.id}
