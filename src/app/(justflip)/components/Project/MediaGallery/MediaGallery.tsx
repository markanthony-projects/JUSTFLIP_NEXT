...
interface MediaGalleryProps {
    ...
    onClose: () => void;
    project: Project;
    modalType?: "images" | "videos" | "floorPlans";
}

export default function MediaGallery({ isOpen, onClose, project, modalType = "images" }: MediaGalleryProps) {
    const media = useMemo(() =>
        getMedia({ project }),
        [project]);

    const [tab, setTab] = useState<"images" | "videos" | "floorPlans">(modalType);
    const [activeItem, setActiveItem] = useState<Media | any>(null);
    ...
    useEffect(() => {
        ...
        setActiveItem((prev: any) =>
            currentList.find((m: any) => m.id === prev?.title) || currentList[0]
        );
    }, [tab, currentList]);
