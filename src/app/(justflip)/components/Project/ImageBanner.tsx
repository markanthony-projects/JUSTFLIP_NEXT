...
function OverlayButton({ label, count }: { label: string; count: number }) {
    return (
        <>
...
function ImageBanner({ project }: { project: Project }) {
    const [modalType, setModalType] = useState<"images" | "videos" | "floorPlans" | undefined>(undefined)
    const [openModal, setOpenModal] = useState(false)
    const { images, videos, floorPlans } = getMedia({ project })
    const getImage = (index: number) => images?.[index]?.url;

    const openModalHandler = useCallback((type: "images" | "videos" | "floorPlans" | undefined) => {
        setModalType(type);
        setOpenModal(true);
    },
            ...
            <div className="hidden md:grid grid-cols-2 gap-1">
                ...
                {videos?.length > 0 && (
                    <div className="relative cursor-pointer rounded overflow-hidden" onClick={() => openModalHandler("videos")} >
                        <video src={videos[0]?.url} className="w-full h-full object-cover rounded-md" muted playsInline preload="metadata" />
