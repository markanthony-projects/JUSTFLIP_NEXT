...
const CompareCarousel = ({ data, isDefault = true }: CompareCarouselProps) => {
    ...
    const renderCard = (item: Project, index: number, isMobileGrid = false) => {
        const bannerFromMedias = item?.medias?.find((m: any) => m.type?.toLowerCase() === "image" && m.title?.toLowerCase() === "banner");
