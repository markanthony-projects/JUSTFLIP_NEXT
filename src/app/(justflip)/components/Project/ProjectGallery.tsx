...
const LocationImageGallery = ({ data }: { data: { name?: string; medias?: Media[]; [key: string]: any } }) => {
    const [activeIndex, setActiveIndex] = useState(0);

    const images = useMemo(() => {
        if (!data?.medias) return [];
        return data.medias.filter((o: Media) => o?.type === "image" && o?.title && ["other", "others"].includes(o.title.toLowerCase()));
    }, [data]);
