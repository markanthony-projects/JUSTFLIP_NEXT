...
export default function MobileSearchModal() {
    ...
    const [suggestions, setSuggestions] = useState<{ projects: any[], builders: any[], locations: any[] }>(emptySuggestions);
    ...
    const toggleLocality = (loc: any) => {
        if (selectedLocalities.find((l: any) => l.id === loc.id)) {
            setSelectedLocalities(selectedLocalities.filter((l: any) => l.id !== loc.id));
        } else {
                            ...
                            {flatSuggestions.length > 0 ? (
                                ...
                                <div className="px-4 py-2">
                                    {Object.values(SEARCH_CONFIG.filters).map((config) => (
                                        <FilterFactory key={config.key} config={config} onClose={() => {}} />
                                    ))}
                                </div>
                            )}
                        </div>
