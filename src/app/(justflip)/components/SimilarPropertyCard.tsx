...
import { createProjectUrl, parseProjectUrl } from "@/src/utils/url";
import Link from "next/link";
import { useRouter, useParams } from "next/navigation";
import { Project, Media } from "@/src/types";
import { MdOutlineLocationOn, MdCompareArrows } from "react-icons/md";
...
const SimilarPropertyCard = ({ project, priority }: { project: Project; priority?: boolean }) => {
    ...
    const { id: baseProjectId } = parseProjectUrl(city, zone, location, slug);

    const { items: compareItems, add: addCompare } = useCompareStore();
    const isCompared = compareItems.some((item) => item.id === project?.id);
    ...
    const handleCompareClick = (e: React.MouseEvent) => {
        ...
        const storeIds = compareItems.map((item) => item.id);
        let targetIds: string[] = [];

        if (storeIds.length > 0) {
            const combinedIdsSet = new Set([...storeIds, project.id]);
            targetIds = Array.from(combinedIdsSet);
        } else {
            const combinedIdsSet = new Set<string>();
            if (baseProjectId) {
                combinedIdsSet.add(baseProjectId);
            }
            combinedIdsSet.add(project.id);
            targetIds = Array.from(combinedIdsSet);
        }

        if (targetIds.length > 3) {
            toast.error("Only 3 properties can be compared at a time");
            return;
        }
        ...
        router.push(`/compare?ids=${targetIds.join(",")}`);
    };
                    ...
                    <div className="h-35 md:h-35 xl:h-35 relative overflow-hidden bg-gray-100 border-b border-gray-100 w-60 md:w-60 max-w-60">
                        ...
                        <div className="absolute bottom-2 right-2 z-10">
                            ...
                            >
                                ...
                                {isCompared ? "Go to Comparison" : "Compare"}
                            </button>
                        </div>
                    </div>
...
export default memo(SimilarPropertyCard);