...
interface UploadZoneProps {
    ...
    onUpload: (name: string, multiple: boolean, files: FileList | null) => void;
}
...
interface BrokerPropertyMediaProps {
    formData: any;
    handleUpload: (name: string, multiple: boolean, files: FileList | null) => void;
    handleRemoveMedia: (idx: number) => void;
    handleMediaMetaChange: (idx: number, field: string, val: string) => void;
    handleFloorPlan: (unitIndex: number, files: FileList | null) => void;
    handleRemoveFloorPlan: (unitIndex: number, imageIdx: number) => void;
    handleFloorPlanAltChange: (unitIndex: number, imageIdx: number, val: string) => void;
    handleFloorPlanUnitChange: (unitIndex: number, imageIdx: number, val: string) => void;
    isUploadingFloorPlans: boolean;
    uploadingCategory: string | null;
}

const BrokerPropertyMedia: React.FC<BrokerPropertyMediaProps> = ({
    formData,
    handleUpload,
    handleRemoveMedia,
        ...
        <div className="space-y-8">
            ...
            {MEDIA_CATEGORIES.map((cat) => (
                ...
                <BiGridAlt className="text-[#002B5B] text-xl mt-0.5 shrink-0" />
                    ...
                    {(formData?.medias || []).filter((m: MediaFile) => m.title === cat.title).length > 0 && (
                        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 overflow-y-auto scrollbar-modern">
                            {(formData?.medias || []).map((file: MediaFile, idx: number) => {
