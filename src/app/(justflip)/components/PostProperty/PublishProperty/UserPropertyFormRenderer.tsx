...
interface UserPropertyFormRendererProps {
    fields?: any[];
    formData?: any;
    errors?: any;
    ...
    setShowProjectDropdown?: (val: boolean) => void;
    onSelectProject?: (project: any) => void;
    onProjectSearch?: (query: string) => void;
    projectQuery?: string;
    locationSuggestions?: any[];
    showLocationDropdown?: boolean;
    setShowLocationDropdown?: (val: boolean) => void;
    onLocationSearch?: (query: string, cityId?: string) => void;
    onSelectLocation?: (loc: any) => void;
    locationQuery?: string;
}

const UserPropertyFormRenderer = ({
    fields = [],
    formData,
    errors,
    handleChange,
            ...
            {fields.map((field: any) => (
                <div key={field.name} className="flex flex-col gap-2">
                    ...
                    {field.type === "project-search" && (
                        <SearchDropdown
                            value={projectQuery || ""}
                            onChange={(q: string) => onProjectSearch?.(q)}
                            suggestions={projectSuggestions}
                            onSelect={(item: any) => onSelectProject?.(item)}
                            showSuggestions={showProjectDropdown || false}
                            setShowSuggestions={(val: boolean) => setShowProjectDropdown?.(val)}
                    ...
                    {field.type === "location-search" && (
                        <SearchDropdown
                            value={locationQuery || ""}
                            onChange={(q: string) => onLocationSearch?.(q, formData?.cityId)}
                            suggestions={locationSuggestions}
                            onSelect={(item: any) => onSelectLocation?.(item)}
                            showSuggestions={showLocationDropdown || false}
                            setShowSuggestions={(val: boolean) => setShowLocationDropdown?.(val)}
...
export default UserPropertyFormRenderer;