...
const UserPropertySideBarStepper = ({ currentStep }) => {
  ...
  return (
    <div className='w-full lg:w-[320px] shrink-0 bg-white rounded-xl p-5 flex-col shadow-[0_8px_30px_rgb(0,0,0,0.08)] hidden md:flex'>
      ...
      <div className='flex flex-col gap-5'>
        ...
        <div className='space-y-0.5 mt-1'>
          <h2 className='text-[20px] font-bold text-[#002B5B] tracking-tight flex items-center gap-2'>
            <FaHome className='text-2xl text-[#002B5B]' />
            Post your property
          </h2>
        </div>
              ...
              <div key={step.id} className='flex  gap-3'>
                ...
                <div className='flex flex-col items-center'>
                  ...
                  <div className='z-10 bg-white py-1'>
                    {isCompleted && (
                      <div className='w-7 h-7 rounded-full bg-[#057748] flex items-center justify-center text-white shadow-[#057748]/20'>
                        <FaCheck className='text-white text-[10px]' />
                      </div>
                    )}
                    {isCurrent && (
                      <div className='w-7 h-7 rounded-full border-[2.5px] border-[#002B5B] flex items-center justify-center bg-white'>
                  ...
                  {!isLast && (
                    <div className='flex-1 w-0.5 min-h-[20px] my-0.5 rounded-full overflow-hidden flex flex-col'>
                      <div
                        className={`h-full ${
                          isCompleted
                            ? 'bg-[#057748]'
                            : isCurrent
                            ? 'bg-linear-to-b from-[#002B5B] to-slate-100'
                            : 'bg-slate-100'
                        }`}
                      ></div>
                    </div>
                ...
                {/* the content column */}
                <div
                  className={`pt-1 pb-4 flex flex-col ${isLast ? 'pb-1' : ''}`}
                >
                  <h3
                    className={`text-[14px] font-bold ${
                      isCurrent ? 'text-[#002B5B]' : 'text-slate-600'
                    }`}
                  >
                    {step.title}
                  </h3>
                  ...
                  <div className='mt-1'>
                    ...
                    {isPending && (
                      <span className='inline-flex px-2 py-0.5 rounded-full text-[11px] font-semibold bg-[#f0f0f0] text-[#6b7280]'>
                        Pending
                      </span>
                    )}
                  </div>
      ...
      <div className='mt-0 pt-4 border-t border-slate-400 flex flex-col gap-4'>
        <p className='w-full text-wrap text-[11.5px] leading-relaxed font-medium text-slate-500 text-center'>
          Need help with the form? Feel free to call or message us on WhatsApp
          anytime!
        </p>
          ...
          <SimpleButton
            ...
            label={
              <span className='flex items-center gap-2'>
                Call For Help <IoCallOutline className='text-sm' />
              </span>
            }
          />
          <SimpleButton
...
export default UserPropertySideBarStepper
