...
interface PublishPropertyMediaProps {
    formData: any;
    handleUpload: (fieldName: string, multiple: boolean, files: FileList | File[]) => void;
    handleRemoveMedia: (index: number) => void;
    handleMediaMetaChange: (index: number, key: string, value: string) => void;
    handleFloorPlan: (unitIndex: number, files: FileList | File[]) => void;
    handleRemoveFloorPlan: (unitIndex: number, imageIndex: number) => void;
    handleFloorPlanAltChange: (unitIndex: number, imageIndex: number, value: string) => void;
    handleFloorPlanUnitChange: (unitIndex: number, imageIndex: number, value: string) => void;
...
const PublishPropertyMedia = ({
    ...
    return (
        <div className="space-y-8">
            {categories.map((cat) => (
                <div key={cat.name} className="p-6 rounded-lg border border-slate-100 bg-slate-50/50 space-y-4">
                    <div className="flex items-start gap-2">
                        ...
                        <div className="relative group">
                            <input
                                ...
                                onChange={(e) => handleUpload(cat.name, cat.multiple, e.target.files as FileList)}
                                className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10"
                            />
                            <div className="w-full py-8 border-2 border-dashed border-slate-300 rounded-lg bg-white flex flex-col items-center justify-center gap-2 group-hover:border-primary group-hover:bg-blue-50/30 transition-all duration-300">
                        ...
                        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                            {formData?.medias?.filter((m: any) => m.title === cat.title).map((file: any, idx: number) => {
                                const originalIdx = formData.medias.indexOf(file);
                                return (
                                    <div key={idx} className="bg-white flex gap-2 border border-slate-200 w-full rounded-lg shadow-sm">
                    ...
                    <FloorPlanUploader
                        ...
                        onUpload={(files: any) => handleFloorPlan(0, files as FileList)}
                        onRemove={(imageIdx: number) => handleRemoveFloorPlan(0, imageIdx)}
                        onAltChange={(imageIdx: number, val: string) => handleFloorPlanAltChange(0, imageIdx, val)}
                        onUnitChange={(imageIdx: number, val: string) => handleFloorPlanUnitChange(0, imageIdx, val)}
