...
// Handle map click
const MapEvents = ({ onMapClick }: { onMapClick: (latlng: any) => void }) => {
  useMapEvents({
    click: (e: any) => {
      onMapClick(e.latlng);
    },
...
const PublishPropertyMap = ({ isOpen, onClose, coordinates, onSave }: { isOpen: boolean, onClose: () => void, coordinates: { lat: number | null, lng: number | null }, onSave: (coords: { lat: number | null, lng: number | null }) => void }) => {
  const DEFAULT_CENTER = { lat: 12.9716, lng: 77.5946 };

  const [tempCoords, setTempCoords] = useState<{ lat: number | null; lng: number | null }>(DEFAULT_CENTER);
