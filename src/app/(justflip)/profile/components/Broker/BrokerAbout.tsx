...
const BrokerAbout = () => {
  ...
  return (
    <div className="bg-white rounded-lg p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 relative overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row gap-8 items-start">
        ...
        <div className="flex flex-col items-center shrink-0 w-full md:w-56">
          ...
          <p className="text-sm font-semibold text-primary text-center mt-1">Broker: {user?.name}</p>
          {user?.rera && (
            <p className="text-[11px] font-semibold text-slate-500 uppercase tracking-wide mt-1">RERA: {user.rera}</p>
          )}
          <div className="mt-3">
            <StarRating value={user?.averageRating} width={4} height={4} />
          </div>
        </div>
                ...
                <div className="flex flex-wrap gap-x-12 gap-y-6 w-full">
                  {user?.expertiesIn?.filter(Boolean)?.length > 0 && (
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {user?.expertiesIn.filter(Boolean).map((exp: string, i: number) => (
                  ...
                  {user?.brokerOperatedCities?.filter(Boolean)?.length > 0 && (
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {user?.brokerOperatedCities.filter(Boolean).map((cityObj: any, i: number) => {
