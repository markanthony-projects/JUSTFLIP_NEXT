...
const BrokerHeader = ({ onEditClick, onUpdateClick }: { onEditClick: () => void, onUpdateClick: () => void }) => {
  ...
  const latestRemark = React.useMemo(() => {
  ...
  const handleImageUpdate = async (e: React.ChangeEvent<HTMLInputElement>, field: string) => {
        ...
        <div className='hidden md:block'>
          ...
          {isRejected && latestRemark && (
            ...
            >
              ...
              <p className='text-sm text-red-800 font-medium leading-relaxed max-w-3xl'>{latestRemark[1] as React.ReactNode}</p>
            </motion.div>
          )}
