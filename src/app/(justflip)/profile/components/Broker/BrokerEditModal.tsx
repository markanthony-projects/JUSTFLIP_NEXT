...
const BrokerEditModal = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast({ message: (error as any)?.response?.data?.error || 'Failed to update profile. API might not be ready.', type: 'error' })
    } finally {
      setIsLoading(false)
    }
  }
