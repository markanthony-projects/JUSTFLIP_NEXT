...
const Wishlist = ({ isEmbedded = false }: { isEmbedded?: boolean }) => {
  ...
  const handleFavoriteSave = async (propertyId: string, item: string = 'project') => {
    if (!isAuthenticated) {
    ...
    return (
      <div className='flex justify-center h-screen overflow-hidden items-center w-full'>
        <div className="flex flex-col items-center">
          ...
          <p className="text-primary font-medium mt-4 animate-pulse">Checking authentication...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={isEmbedded ? 'p-4 md:p-5 w-full h-full' : 'px-4 md:px-8 xl:px-14 lg:px-12 w-full 2xl:max-w-[1540px] mx-auto overflow-x-hidden pt-[10px] pb-10'}>
      {!isEmbedded && <Breadcrumb items={breadcrumbItems} />}

      <div className={`${isEmbedded ? 'mt-0 mb-6' : 'mt-8 mb-8'} flex justify-between items-end border-b border-gray-200 pb-4`}>
        <div>
          <h1 className="text-3xl md:text-4xl font-bold text-primary tracking-tight">My Wishlist</h1>
        ...
        <div className="bg-[#E6EEF2] px-4 py-2 rounded-full hidden sm:block">
          <p className="text-primary font-semibold text-sm">{datas?.length || 0} Saved Properties</p>
        </div>
      </div>
      ...
      <div className='mt-5'>
        {datas?.length > 0 &&
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-x-6 gap-y-10 h-auto min-h-[50vh]">
            {datas?.map((property: any, index: number) => {
              const { minPrice, maxPrice } = getLowestAndHighestPrice(property?.units);
              const bannerUrl = property?.medias?.find((media: any) => media.title === "banner")?.url || property?.bannerImage;
              ...
              return (
                <Link href={`/properties/${property?.city?.name}/${property?.zone?.name}/${property?.location?.name}/${property?.name}-${property?.id}`} key={property?.id}>
                  <div className="group relative rounded-lg transition-all duration-300 ease-in-out transform hover:-translate-y-2 hover:shadow-2xl bg-white border border-gray-100 flex flex-col overflow-hidden h-full">
                    ...
                    <div className="relative w-full h-[15rem] bg-gray-100 shrink-0 overflow-hidden">
                      ...
                      {/* Logo (Top Left) */}
                      {logoUrl && (
                        <div className="absolute top-4 left-4 bg-white p-1.5 rounded-lg shadow-md z-10 transform transition-transform duration-300 group-hover:scale-105">
                          <img
                            src={logoUrl}
                            alt={property?.name}
                            loading="eager"
                      ...
                      <div className="absolute bottom-4 left-4 bg-white/95 backdrop-blur-md px-4 py-1.5 rounded-full text-xs font-bold text-primary shadow-lg z-10 transform transition-transform duration-300 group-hover:-translate-y-1">
                        Exclusive
                      </div>
                    </div>
            ...
            <Link href="/" className="flex flex-col items-center justify-center border-2 border-dashed border-primary/30 rounded-lg min-h-[22.5rem] bg-gradient-to-b from-gray-50 to-white hover:border-primary transition-all duration-300 group shadow-sm hover:shadow-lg">
              <div className="w-16 h-16 rounded-full bg-white border-2 border-primary/20 group-hover:border-primary group-hover:bg-primary flex items-center justify-center text-primary/40 group-hover:text-white mb-4 transition-all duration-500 transform group-hover:rotate-90 group-hover:scale-110 shadow-sm">
                <AiOutlinePlus size={28} />
              </div>
              <span className="text-primary/70 font-semibold text-base group-hover:text-primary transition-colors">Add more properties</span>
        ...
        {(!loading && datas?.length === 0) &&
          <div className='flex flex-col justify-center h-[60vh] items-center w-full bg-gray-50 rounded-lg border border-gray-100'>
            ...
            <Link href="/" className="bg-primary text-white px-8 py-3 rounded-full font-semibold hover:bg-[#001f42] hover:shadow-lg hover:-translate-y-1 transition-all duration-300 flex items-center gap-2">
              Browse Properties
            </Link>
          </div>
        }
        {loading && (
          <div className='flex justify-center h-[50vh] overflow-hidden items-center w-full'>
            <div className="flex flex-col items-center">
              <div className="animate-spin rounded-full h-12 w-12 border-4 border-gray-200 border-t-primary"></div>
              <p className="text-primary font-medium mt-4 animate-pulse">Loading your favorites...</p>
            </div>
          </div>
        )}
      </div>
