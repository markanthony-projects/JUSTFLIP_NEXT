...
// helper functions
const buildhref = (type: string, data: any) => {
    if(type === 'project') {
...
//  const router = useRouter() //to route
  ...
  const debounceRef = useRef<NodeJS.Timeout | null>(null) //holds the setTimeout id for debounceing.
  const containerRef = useRef<HTMLDivElement>(null) //it refers to the whole search bar wrapper.
  ...
  const debouncedFetch = useDebounce((search: string) => {
    if (!search) {
      setSuggestions(emptySuggestions)
      return
    }
